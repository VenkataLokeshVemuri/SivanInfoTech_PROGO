...
const CourseEnrollment = () => {
  ...
  const fetchCourses = async () => {
    try {
      ...
      if (response.success && details && Array.isArray(details)) {
        setCourses(details as Course[]);
      } else {
    ...
    <div className="space-y-12 px-4 md:px-8 max-w-5xl mx-auto">
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-gray-700 text-lg">
          Choose from our comprehensive cloud training programs
        </p>
      </div>
      ...
      {!isAuthenticated && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
          <p className="text-yellow-800 text-center">
            <strong>Note:</strong> You can browse courses as a guest, but you'll
            need to provide your details during enrollment.
          </p>
        </div>
      )}
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    {course.batches?.length || 0} batch
                    {course.batches?.length !== 1 ? "es" : ""} available
                  </p>
                </div>
                ...
                <div className="px-6 pb-6 space-y-4 border-t border-gray-100">
                  {course.batches && course.batches.length > 0 ? (
                    course.batches.map((batch) => {
                      const seatsLeft = calculateSeatsLeft(
                        batch.capacity,
                        batch.enrolled
                      );
                      const isFullBatch = isBatchFull(
                        batch.capacity,
                        batch.enrolled
                      );
                        ...
                        >
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                            ...
                            <div className="flex items-center text-purple-600">
                              ...
                              <span>
                                {batch.timing || "Timing not available"}
                              </span>
                            </div>
