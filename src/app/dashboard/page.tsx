...
import { 
  User, Award, BookOpen, Cloud, LogOut, Mail, Calendar, Clock, 
  TrendingUp, Star, Settings, BarChart3, GraduationCap, Menu, X,
  ChevronDown, Bell, Search, Sparkles, ArrowRight 
} from 'lucide-react';
...
const StudentNavbar = ({ userData, onLogout }: { userData: UserData; onLogout: () => void }) => {
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <Link href="/" className="flex items-center space-x-3 group">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xs text-gray-500 font-medium -mt-1">Learning Platform</span>
            </div>
          </Link>
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="center" className="w-80 p-0 border-0 shadow-xl bg-white rounded-2xl overflow-hidden">
                <div className="bg-gradient-to-r from-blue-600 to-green-600 p-4 text-white">
                  ...
                  <p className="text-sm opacity-90">{userData.enrollments?.length || 0} active enrollments</p>
                </div>
                <div className="p-4 max-h-96 overflow-y-auto">
                  {userData.enrollments && userData.enrollments.length > 0 ? (
                    <div className="space-y-3">
                      {userData.enrollments.map((enrollment) => (
                        <div key={enrollment.enrollmentID} className="p-3 rounded-lg border border-gray-100 hover:border-blue-200 hover:bg-blue-50/50 transition-all duration-200">
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <h4 className="font-medium text-gray-900 text-sm">{enrollment.courseTitle}</h4>
                              <p className="text-xs text-gray-500 mt-1">Course ID: {enrollment.courseShortForm}</p>
                              <div className="flex items-center mt-2">
                                <Calendar className="h-3 w-3 text-gray-400 mr-1" />
                                <span className="text-xs text-gray-500">Enrolled: {new Date(enrollment.enrolledDate).toLocaleDateString()}</span>
                              </div>
                            </div>
                            <Badge className={
                              enrollment.enrollmentStatus === 'Approved' ? 'bg-green-100 text-green-800 text-xs' :
                              enrollment.enrollmentStatus === 'Certified' ? 'bg-purple-100 text-purple-800 text-xs' :
          ...
          <div className="flex items-center space-x-4">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="flex items-center space-x-3 p-2 hover:bg-white/60 rounded-2xl transition-all duration-200">
...
const Dashboard = () => {
  ...
  const getEnrollmentStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'Certified':
        return <Badge className="bg-purple-100 text-purple-800 border-purple-200">Certified</Badge>;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-700 font-medium">Loading your premium dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  const progressPercentage = totalEnrollments > 0 ? Math.round((completedCourses / totalEnrollments) * 100) : 0;
        ...
        <div className="relative overflow-hidden">
          ...
          <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-16">
            ...
            <div className="text-center mb-12">
              <div className="inline-flex items-center px-4 py-2 bg-white/60 backdrop-blur-xl border border-white/20 rounded-full text-sm font-semibold text-blue-600 mb-6 shadow-lg">
                <Star className="mr-2 h-4 w-4" />
                Premium Learning Dashboard
              </div>
              <h1 className="text-4xl md:text-6xl font-black bg-gradient-to-r from-blue-600 via-purple-600 to-green-600 bg-clip-text text-transparent mb-4">
                Welcome back, {userData.firstName}!
              </h1>
              <p className="text-xl text-gray-600 max-w-2xl mx-auto">
                Continue your cloud certification journey with personalized insights and premium features
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
              ...
              <Card className="relative overflow-hidden border-0 bg-white/60 backdrop-blur-xl shadow-2xl hover:shadow-3xl transition-all duration-500 group">
                ...
                <CardContent className="relative z-10">
                  ...
                  <div className="text-xs text-blue-600 mt-2">Active learning paths</div>
                </CardContent>
              </Card>
              ...
              <Card className="relative overflow-hidden border-0 bg-white/60 backdrop-blur-xl shadow-2xl hover:shadow-3xl transition-all duration-500 group">
                ...
                <CardContent className="relative z-10">
                  <div className="text-3xl font-black text-gray-900 mb-1">{completedCourses}</div>
                  <div className="text-sm font-medium text-gray-600">Certificates Earned</div>
                  <div className="text-xs text-green-600 mt-2">Professional achievements</div>
                </CardContent>
              </Card>
              ...
              <Card className="relative overflow-hidden border-0 bg-white/60 backdrop-blur-xl shadow-2xl hover:shadow-3xl transition-all duration-500 group">
                ...
                <CardContent className="relative z-10">
                  <div className="text-3xl font-black text-gray-900 mb-1">{progressPercentage}%</div>
                  <div className="text-sm font-medium text-gray-600">Overall Progress</div>
                  <div className="text-xs text-purple-600 mt-2">Learning completion</div>
                </CardContent>
              </Card>
            </div>
          </div>
              ...
              <Card className="border-0 bg-white/60 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden">
                <CardHeader className="bg-gradient-to-r from-blue-600 to-green-600 text-white p-8">
                  ...
                  <CardDescription className="text-blue-100">
                    Track your learning progress and course status with premium insights
                  </CardDescription>
                </CardHeader>
                <CardContent className="p-8">
                  {userData.enrollments && userData.enrollments.length > 0 ? (
                    <div className="space-y-6">
                      {userData.enrollments.map((enrollment) => (
                        <Card key={enrollment.enrollmentID} className="border-0 bg-gradient-to-r from-white via-gray-50 to-white shadow-lg hover:shadow-xl transition-all duration-300 rounded-2xl overflow-hidden group">
                              ...
                              <div className="text-right">
                                ...
                                {enrollment.certifiedOn && (
                                  <div className="text-xs text-gray-500 mt-2">
                                    Certified: {new Date(enrollment.certifiedOn).toLocaleDateString()}
                                  </div>
                                )}
                              </div>
                            </div>
                    ...
                    <div className="text-center py-16">
                      ...
                      <p className="text-gray-600 mb-6">Start your cloud certification journey today</p>
                      <Link href="/courses">
