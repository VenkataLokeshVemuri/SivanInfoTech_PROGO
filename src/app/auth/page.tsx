...
const Auth = () => {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // result shape: { success: boolean, user?: User, error?: string }
      if (result.success && result.user) {
        // Redirect to home page for both admin and student users
        router.push("/");
        return;
      }
      // If backend returned success but user object is missing, still redirect
      if (result.success) {
        console.warn(
          "Login succeeded but no user object returned; redirecting anyway."
        );
        try {
          router.push("/");
        } catch (err) {
          // fallback for environments where router.push might not navigate as expected
          window.location.href = "/";
        }
        return;
      }
      // If here, login failed - show a friendly message
      const msg =
        (result as any).error || "Login failed. Please check credentials";
      alert(msg);
    } catch (err) {
      // handle error
    }
